...
def traffic():
    ...
    total_vehicles_all = 0 #initialize

    for i in range(1,total_roads_surveyed + 1):
        print("How many days was road", i,end=" ")
        days_surveyed = eval(input("surveyed?"))
        total_vehicles_road = 0 #initialize

        for j in range(1,days_surveyed+1):
            print("\t","How many cars traveled on day", j, end="")
            total_vehicles_day = eval(input("?"))
            total_vehicles_road = total_vehicles_road + total_vehicles_day #count

        avg_vehicles_day = total_vehicles_road / days_surveyed #calculate
        avg_vehicles_day = round(avg_vehicles_day,1) #round
        total_vehicles_all = total_vehicles_all + total_vehicles_road #count
        print("Road", i, "average vehicles per day:", avg_vehicles_day)

    avg_vehicles_road = total_vehicles_all / total_roads_surveyed #calculate
    avg_vehicles_road = round(avg_vehicles_road,2) #round
